...
JSON-файлы служат локальной страховочной копией. После каждой операции записи
плагин отправляет идемпотентный снимок в FastAPI, который обновляет PostgreSQL
с сохранением исходных идентификаторов.

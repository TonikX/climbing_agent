...
- Ручной тестовый режим хранится в PostgreSQL на уровне пользователя. Пока он включён через меню `/journal`, backend принудительно ставит `is_test=true` всем новым попыткам.

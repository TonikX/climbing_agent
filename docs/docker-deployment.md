...
FastAPI содержит health/readiness и транзакционную реализацию всех восьми
инструментов OpenClaw: тренировки, пролазы, каталог трасс и снаряжение.
PostgreSQL — единственный источник данных журнала. Схема создаётся Alembic.
...
Перед обновлением создайте `pg_dump` базы. После запуска OpenClaw выполняет все
операции через внутренний endpoint `/api/v1/tools/{operation}` с `X-API-Key`.
Если API или PostgreSQL недоступны, инструмент возвращает ошибку и не сообщает
